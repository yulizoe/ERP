...
$.extend({
	...
	registerCheck:function(){
		...
		var ok1 = false;var ok2 = false;var ok3 = false;var ok4 = false;var ok5 = false;var ok6 = true;
        ...
        var usernameCheck=function(_this1){
        	...
        	if($(_this1).val().search(/\s+/)==0||$(_this1).val()==''){ showError($(_this1)); ok1=false; } 
		        ...
		        var user=$(_this1).val();
		        $.post("registerInfoAction!isExistsUser.action",{"userID":user},function(data){
		        	var val=data.existCode;
                    if(val==true) {$(_this1).next().empty().append("用户名已存在").css("color","orange").css("padding-left","8px"); } 
	    ...
	    var checkAll=function(){
	    	usernameCheck($('input[name="userID"]'));
	    ...
	    $('input[name="userID"]').blur(function(){ _this1=$(this); usernameCheck(_this1); });
	    $('input[name="name"],input[name="major"],input[name="password"]').blur(function(){ _this2=$(this); rmpCheck(_this2); });
	    ...
	    $('input[name="register"]').click(function(){
	    	if(ok1&&ok2&&ok3&&ok4&&ok5&&ok6) { $('form').submit(); alert("registered"); }
	        else { alert("请输入正确信息"); return false; }
	    });
	}
});